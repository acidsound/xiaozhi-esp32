...
const ACTION_NAMES = {
  '1': '엎드리기', '2': '인사', '3': '기울이기', '4': '흔들기',
  '5': '좌우 흔들기', '6': '살랑이기', '7': '덜덜 떨기', '8': '쿡 찌르기',
  '9': '차기', '10': '점프 →', '11': '← 점프', '12': '접기',
  'F': '앞으로', 'B': '뒤로', 'L': '왼쪽 회전', 'R': '오른쪽 회전'
};
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorData.error === "Control disabled in calibration mode") {
        throw new Error('보정 모드에서는 조작할 수 없습니다');
      }
...
class CustomActionPanel {
  ...
  addAction() {
    ...
    if (!action) {
      this.customError.textContent = '동작을 선택하세요';
      return;
    }

    if (isNaN(delay) || delay < 0 || delay > 10) {
      this.customError.textContent = '대기 시간은 0-10초 사이여야 합니다';
      return;
    }

    if (this.sequence.length >= 4) {
      this.customError.textContent = '동작은 최대 4개까지 추가할 수 있습니다';
      return;
    }
  ...
  saveSequence() {
    try {
      ...
      this.customError.textContent = '동작 조합을 저장했습니다';
    } catch (error) {
      this.customError.textContent = '저장하지 못했습니다';
    }
  }
  ...
  loadSavedSequence() {
    try {
      ...
      console.error('저장된 동작 조합을 불러오지 못했습니다:', error);
    }
  }
}
...
class SettingsPanel {
  ...
  async loadBraveKeyStatus() {
    try {
      ...
      this.webKeyStatus.textContent = '알 수 없음';
      this.llmContextKeyStatus.textContent = '알 수 없음';
      this.error.textContent = error.message;
    }
  }

  updateKeyStatus(element, configured) {
    element.textContent = configured ? '설정됨' : '미설정';
  ...
  async clearWebKey() {
    try {
      ...
      this.updateStatuses(data);
      this.useLlmContextInput.checked = Boolean(data.use_llm_context);
      this.error.textContent = 'Web Search 키를 삭제했습니다';
    } catch (error) {
      this.error.textContent = error.message;
    }
  }
  ...
  async clearLlmContextKey() {
    try {
      ...
      this.updateStatuses(data);
      this.useLlmContextInput.checked = Boolean(data.use_llm_context);
      this.error.textContent = 'LLM Context 키를 삭제했습니다';
    } catch (error) {
      this.error.textContent = error.message;
    }
  }
  ...
  async saveLlmContextSetting() {
    try {
      ...
      this.updateStatuses(data);
      this.useLlmContextInput.checked = Boolean(data.use_llm_context);
      this.error.textContent = this.useLlmContextInput.checked ? 'LLM Context를 켰습니다' : 'LLM Context를 껐습니다';
    } catch (error) {

...
static TickType_t servo_dog_delay_ticks_or_min(TickType_t delay_ticks)
{
    return delay_ticks > 0 ? delay_ticks : 1;
}

#define SERVOR_DOG_ACTION_DELAY(delay_ticks) \
    if (xQueuePeek(g_servo_dog->dog_action_queue, &g_servo_dog->msg, servo_dog_delay_ticks_or_min(delay_ticks)) == pdTRUE) { \
        return; \
    } \

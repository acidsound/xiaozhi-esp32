...
class SettingsPanel {
  ...
  async loadBraveKeyStatus() {
    try {
      ...
      this.webKeyStatus.textContent = 'Unknown';
      this.llmContextKeyStatus.textContent = 'Unknown';
  ...
  async clearWebKey() {
    try {
      ...
      this.updateStatuses(data);
      this.useLlmContextInput.checked = Boolean(data.use_llm_context);
      this.error.textContent = 'Web key cleared';
    } catch (error) {
      this.error.textContent = error.message;
    }
  }
  ...
  async clearLlmContextKey() {
    try {
      ...
      this.updateStatuses(data);
      this.useLlmContextInput.checked = Boolean(data.use_llm_context);
      this.error.textContent = 'LLM Context key cleared';
    } catch (error) {
      this.error.textContent = error.message;
    }
  }
  ...
  async saveLlmContextSetting() {
    try {
      ...
      this.updateStatuses(data);
      this.useLlmContextInput.checked = Boolean(data.use_llm_context);
      this.error.textContent = this.useLlmContextInput.checked ? 'LLM Context enabled' : 'LLM Context disabled';
    } catch (error) {
